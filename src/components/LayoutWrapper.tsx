...
export default function LayoutWrapper({ children }: { children: React.ReactNode }) {
  ...
  // Fetch Supabase user data if not on a public page
  useEffect(() => {
    ...
    const fetchUser = async () => {
      try {
        ...
        if (data?.user) {
          setUser(data.user);
          // Auto fill profile from Supabase metadata if exists
          const fullName = data.user.user_metadata?.full_name || data.user.email || "Akash Camera Admin";
          setProfile(prev => ({
            ...prev,
  ...
  // Load local session storage and backend url on mount
  useEffect(() => {
  ...
  const getInitials = (name?: string) => {
    const displayName = name || user?.user_metadata?.full_name || user?.email || profile.name || "";
      ...
      <div className="min-h-screen flex w-full bg-background text-foreground font-sans overflow-hidden transition-colors duration-200">
        ...
        <main className="flex-1 ml-0 md:ml-20 flex flex-col h-screen overflow-hidden bg-slate-50 dark:bg-slate-950 transition-colors duration-200 pb-16 md:pb-0">
          ...
          <header className="h-14 glass-header border-b border-gray-200/50 dark:border-slate-800/50 sticky top-0 z-10 flex items-center justify-between px-6 shadow-sm shrink-0">
            ...
            <div className="flex items-center gap-4 text-slate-500 dark:text-slate-400 relative">
