        ...
        io.on('connection', (socket) => {
            ...
            socket.on('get_projects', async () => {
                try {
                    ...
                    // Filter in memory to ONLY show groups that belong to the currently active WhatsApp account (only when ready and synced)
                    let filteredProjects = data || [];
                    if (currentStatus === 'ready' && activeGroupsList.length > 0) {
                        filteredProjects = (data || []).filter(proj => activeGroupsList.includes(proj.whatsapp_group_id));
                    }
...
async function syncGroups(client, io) {
    ...
    try {
        ...
        const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('Timeout fetching chats')), 60000));
        ...
        for (const chat of chats) {
            ...
            const chatData = {
                ...
                group_name: chatName || chat.id.user,
                status: 'Unassigned',
                event_month: chat.isGroup ? (extractDateFallback(chat.name) || 'Unknown') : 'Direct Chat',
                created_at: new Date()
            };
            ...
            if (process.env.SUPABASE_URL) {
                try {
                    await supabaseWithRetry(() => 
                        supabase
                            .from('projects')
                            .upsert([chatData], { onConflict: 'whatsapp_group_id' })
                    );
                } catch (err) {
                    console.error(`Supabase Error on chat ${chatName || chat.id.user}:`, err.message);
                }
            }
            syncedCount++;
        }
        ...
        try {
            ...
            const filtered = (data || []).filter(proj => activeGroupsList.includes(proj.whatsapp_group_id));
            if (io) {
